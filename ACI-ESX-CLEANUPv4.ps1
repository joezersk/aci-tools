...
#Get available local vswitch portgroups from the chosen ESX host
   #$VMNets = Get-VMHost -Name $ChosenVMHost | Get-VirtualPortGroup -Standard
    $VMnets = Get-VirtualSwitch -VMHost $ChosenVMHost | Get-VirtualPortGroup -Standard
...
$password = Read-Host "What is the admin password?" -AsSecureString

# Convert SecureString to pass usable password to APIC
