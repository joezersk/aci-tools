...
try
{
$r = Invoke-RestMethod -Uri $url -Method Post -SessionVariable s -Body $creds -TimeoutSec 5 -ErrorAction Stop
$cookies = $s.Cookies.GetCookies($url)
}
Catch
{
    $ErrorMessage = $_.Exception.Message
    Write-Host "`nI could not seem to successfully log in to your APIC.  Did you give the right IP and Creds?
    `nI will terminate the script here so you can try again." -ForegroundColor Red
    Break
}
...
write-host -ForegroundColor Yellow "`nYour APIC is running version" $apicver.firmwareCtrlrRunning.attributes.version "- Please make sure your installed Agents are the same version."

#Check for presence of SCVMM Agent installed on the local machine and prompt to install if missing
Write-Host "`nOk, checking if you already installed the SCVMM Agent on this machine..." -ForegroundColor Green
...
Else
{Write-Host "`nService $Service does not exist and we need to install it....
...
You can also just drag/drop the file to the command line here`n"
...
New-ApicOpflexCert -ValidNotBefore 1/1/2015 -ValidNotAfter 1/1/2025 -Email $email -Country $country -State $state -Locality "$locality" -Organization $myorg -PfxPassword $pfxpassword
...
Write-Host "`nHere is the cert output above.  I will copy this into the APIC admin account's x.509 certificate object for you." -foreground "yellow"
...
Write-Host "`nOk, the cert is now copied over to APIC.  Now let's copy the PFX file over to your Hyper-V hosts.
...
ForEach ($h in $HyperHost)
{
 if (-not(Test-Connection -ComputerName $h -Count 1 -Quiet))
    {
    Write-Host "`nHost $h unavailable so I skipped it." -ForegroundColor Yellow
    }
else {
    Try 
    {
    ...
    Write-host "`nChecking $h for the presence of the HyperV Agent Service...
APIC HyperV Service Exists and is running!" -ForegroundColor Green
    }
Catch 
    {
    $reachable = $false
    ...
    For the HyperV Host called $h...ACI Hyper-V Agent Not Installed or Not Running.  
    I will copy the certificate anyway...but you need to go back and manually install/start 
    the HyperV agent on the host for APIC communication to work.
...
Write-Host ""
Try {
Set-ApicConnInfo -ApicNameOrIPAddress $apic -CertificateSubjectName OpflexAgent -ErrorAction Stop
Get-ApicConnInfo | Format-Table ApicAddresses, ConnectionStatus 

Write-Host "Ok, the whole process is now complete if you see the Connection Status as CONNECTED." -ForegroundColor Green
    }
Catch {
Write-Host "`nCould not establish a connection between SCVMM and APIC. Check agent services are running and try again." -ForegroundColor Red
    }
