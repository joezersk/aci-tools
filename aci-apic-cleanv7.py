...
myApic = ["10.50.129.241","10.50.129.239","1.1.1.1"]
...
#                result = shell.run(["acidiag", "touch", "setup"])
                result = shell.spawn(["acidiag", "reboot"])
                result.stdin_write("y")
                print (bcolors.OKGREEN + "\nErasing and reloading this APIC now..." + bcolors.ENDC)
                continue
            except spur.ssh.ConnectionError as error:
                print (bcolors.FAIL + "\nAPIC " + myApic[i] + " is pingable but I cannot seem to SSH succesfully.  Moving on..." + bcolors.ENDC)
        else:
            print (bcolors.FAIL +  "\n" + myApic[i] + " - APIC is not reachable, moving on..." + bcolors.ENDC)
main()
